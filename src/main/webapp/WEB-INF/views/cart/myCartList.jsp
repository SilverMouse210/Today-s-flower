...
var totalqty=0;
function calcGoodsPrice(flowersPrice,obj,index){
	...
	var t_deliveryPrice = document.getElementById("t_deliveryPrice");
	var product_price=Number(goods_qty.value*flowersPrice);
	...
	if(obj.checked==true){
		...
		totalPrice=Number(h_totalPrice.value)+Number(goods_qty.value*flowersPrice);//상품 총금액
	...
	if(totalPrice<100000){
		...
		t_deliveryPrice.innerHTML="<p class='fs-3'>"+h_totalDelivery2.value+"원</p><small>(10만원이상 배송비 무료)</small>";
		
		
		
		}else{
			t_deliveryPrice.innerHTML="<p class='fs-3'>10만원 이상 구매시 배송비 무료!</p>";
		}
...
function modify_cart_qty(goods_id,flowersPrice,index){
	console.log(index);
		...
		success : function(data, textStatus) {
			...
			if(result.trim()=='modify_success'){
				alert("수량을 변경했습니다!!");
				 ...
				 totalqty=jsonInfo.qty;
				
				var html ='';
				
			html += "<p id='p_totalGoodsNum' class='fs-3'>"+totalqty+"개 </p>";
				html += "<input id='h_totalGoodsNum'type='hidden' value='"+totalqty+"'/>";
				var newTOtalNum = document.getElementById("newTOtalNum");
				
				newTOtalNum.innerHTML = html;
				
				/*var newGoodsPrice=$('#item_goods_price').val*totalqty;
			    var html2 =''; <!-- 합계변경-->
			    html2 +="<p class='fs-5 mt-5' id='price${cnt.count-1 }'>"+newGoodsPrice+"원 </p>";
			    html2 +="<input type='hidden' id='p_totalPric' value='"+newGoodsPrice+"' />";
			    */
					...
					<tr>
				     ...
				     <c:set var="cart_id" value="${myCartList[cnt.count-1].cart_id}" />
						...
						<td><fmt:formatNumber  value="${item.goods_sales_price*cart_goods_qty}" type="number" var="total_sales_price" />
				         ...
				         <input type="hidden" id="item_goods_price" value="${item.goods_sales_price}" />
				         </td>
				         <!-- 가격 -->
