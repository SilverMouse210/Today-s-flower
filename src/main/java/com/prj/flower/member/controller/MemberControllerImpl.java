...
public class MemberControllerImpl extends BaseController implements MemberController{
	...
	//이메일 인증
		...
		public ResponseEntity navermailtest(@RequestParam("email") String email, HttpServletRequest request, HttpServletResponse response) throws Exception{
	        ...
	        final String username = "kkjk6310";       //보내는쪽 id
	        final String password = "anwlroekfl00";   //보내는쪽 pwd
	        int port=465;
	        
	        // 메일 내용
	        String recipient = email;    
	        String subject = "[오늘의 꽃] 본인인증을 위해 인증번호를 확인해주세요!"; //제목
		...
		public ResponseEntity findMemberId(@RequestParam("email") String email, @RequestParam("email2") String email2,@RequestParam("name") String name, HttpServletRequest request, HttpServletResponse response) throws Exception{
			...
			map.put("email",email);
			map.put("col1",name);
			map.put("col2","member_name");
			String memberId= memberService.findMemberid(map);
			if(memberId!=null) {
			
			// 메일 관련 정보
			String host = "smtp.naver.com";
			final String username = "kkjk6310";       
			final String password = "anwlroekfl00";   
			int port=465;
			email+=email2;
			// 메일 내용
			String recipient = email;    
			String subject = "[오늘의 꽃] 아이디입니다.";
...
//	       
			...
			String htmlText ="회원님의 아이디는 <h1>"+memberId+"</h1>입니다";
			...
			public ResponseEntity findMemberPwd(@RequestParam("email") String email, @RequestParam("email2") String email2, @RequestParam("id") String id,HttpServletRequest request, HttpServletResponse response) throws Exception{
				
				//가입정보 찾기
				Map<String,Object> map = new HashMap<>();
				map.put("email",email);
				map.put("col1",id);
				map.put("col2","member_id");
				
				String memberId= memberService.findMemberid(map);
				if(memberId!=null) {
					email+=email2;
					return navermailtest(email,request,response);}
				
				else return new ResponseEntity(HttpStatus.BAD_REQUEST); 
			}

			
			@RequestMapping(value="/chageMemberPwd.do",method = RequestMethod.POST)
			public ResponseEntity chageMemberPwd(@RequestParam("id") String id, @RequestParam("pwd") String pwd, HttpServletRequest request, HttpServletResponse response) throws Exception{
				
				Map<String,Object> map = new HashMap<>();
				map.put("id",id);
				map.put("member_pw",pwd);
				System.out.println("비밀번호"+ pwd);
				boolean result= memberService.updatePassword(map);
